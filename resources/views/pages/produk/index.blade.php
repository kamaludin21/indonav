...
@push('header')
  <title>Produk | Bangun Dunia Cerdas dengan Solusi Presisi dari INDONAV</title>
...
@section('content')
  <div class="bg-zinc-100">
    <div class="max-w-6xl px-4 md:px-0 mx-auto py-32 space-y-12">
      <div class="capitalize text-center my-8">
        <p class="text-5xl text-slate-800 font-bold">Jelajah <span class="text-orange-500">Produk</span> Kami</p>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        @foreach ($industry as $item)
          <div class="flex h-full rounded-none border border-slate-300 bg-white overflow-hidden group">
            <div class="w-1/2">
            ...
            <div class="w-1/2 p-2 lg:p-6 space-y-2">
              <p class="text-xl font-semibold text-slate-800">{{ $item->title }}</p>
              <p class="text-slate-600">{{ $item->description }}</p>
              <a href="/produk/{{ $item->slug }}"
                class="border-b-2 w-fit flex items-center gap-2 hover:gap-3 duration-200 text-slate-600 hover:text-orange-500">
