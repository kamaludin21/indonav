          ...
          <div class="space-y-4 ">
            ...
            @if (is_array($item->specifications))
              @foreach ($item->specifications as $specs)
                @php
                  $extension = pathinfo($specs['document'], PATHINFO_EXTENSION);
                @endphp
                <div class="space-y-0">
                  <p class="text-lg text-slate-600 font-medium">{{ $specs['title'] }}.{{ $extension }}</p>
                  <a href="{{ asset('storage/' . $specs['document']) }}" target="_blank"
